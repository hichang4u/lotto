    ...
    return (
        <div className="rounded-[24px] border border-slate-200/60 bg-white px-4 py-4 shadow-sm sm:px-5">
            <div className="flex items-start justify-between gap-3">
                <div>
                    <div className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">우선순위 {index + 1}</div>
                    <div className="mt-2 text-base font-bold text-slate-800">{item.label}</div>
                </div>
            ...
            <div className="mt-4 space-y-2">
                <div>
                    ...
                    <div className="h-2 rounded-full bg-slate-100">
                        <div className="h-2 rounded-full bg-slate-400" style={{ width: `${Math.max(item.score * 100, 6)}%` }} />
                    </div>
                </div>
